...
export async function seedImmersiveTraffic() {
  ...
  // Check if it exists
  let projectId;
  const { data: existing } = await supabase.from('projects').select('id').eq('slug', 'immersive-traffic-command').single();
  ...
  console.log('Project created/updated with ID:', projectId);
  ...
  console.log('Successfully seeded Immersive Traffic Command!');
  return true;
}

seedImmersiveTraffic().catch(console.error);
