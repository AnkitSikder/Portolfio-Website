...
export default function ConstellationGrid() {
    ...
    useEffect(() => {
        ...
        const render = (now: number) => {
            // Use fixed timestep for stable physics on all refresh rates (prevents jitter)
            const dt = 0.016;
            lastTime = now;
            ...
            // Use portfolio's background color #171621 instead of #030407
            const bgColor = isDarkMode ? '#171621' : '#f8fafc';
            const nodeColor = isDarkMode ? '255, 255, 255' : '15, 23, 42';
            // Use portfolio's accent orange color #ff5f1f instead of cyan
            const accentColor = isDarkMode ? '255, 95, 31' : '255, 95, 31'; 
    ...
    return (
        <div className="absolute inset-0 z-0 w-full h-full overflow-hidden select-none">
