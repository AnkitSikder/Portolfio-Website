  ...
  return (
    <section id={sectionId} className={`py-12 md:py-20 lg:py-24 px-5 md:px-12 lg:px-24 ${isAlternate ? 'bg-foreground/5' : 'bg-background'} text-foreground w-full overflow-hidden`}>
      <div className="max-w-[1400px] mx-auto flex flex-col gap-8 md:gap-12">
        ...
        <ScrollReveal>
          <SectionHeader title="Style Guide" heading={heading} className="!mb-0" />
        </ScrollReveal>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-10 gap-x-6">
                  {typography.weights?.map((weight, idx) => (
                    <div key={idx} className="flex flex-col items-start gap-2">
                      ...
                      <span className="text-sm text-white/60 font-clash font-medium tracking-wide leading-relaxed">{weight}</span>
                    </div>
                  ))}
                </div>
              </ScrollReveal>
              ...
              <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 w-full items-stretch">
                {marginSpacing.items.map((item, idx) => (
                  <div key={idx} className={`flex flex-col gap-6 w-full ${idx === 0 ? 'lg:w-[70%]' : 'lg:w-[30%]'}`}>
                    ...
                    {/* Stats */}
                    <div className="font-clash font-medium flex flex-row flex-wrap justify-between items-center w-full px-2 text-white/60 gap-4 tracking-wide leading-relaxed">
                      {item.stats?.map((stat, sIdx) => (
                        <div key={sIdx} className="flex flex-row items-baseline gap-2">
                          ...
                          <span className="text-white/60 font-clash font-medium text-md tracking-wide leading-relaxed">{stat.value}</span>
                        </div>
                      ))}
                    </div>
