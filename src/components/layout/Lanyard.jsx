...
// atlas and the back face to the RIGHT half (measured from global/card.glb). Each
...
export default function Lanyard({
  ...
  return (
    <div className="lanyard-wrapper">
      <Canvas
        camera={{ 
          position: isMobile ? [position[0], position[1] - 4, position[2]] : position, 
          fov: isMobile ? 14 : fov 
        }}
...
function Band({
  ...
  const { nodes, materials } = useGLTF('/global/card.glb');
  const texture = useTexture(lanyardImage || '/global/lanyard.png');
  ...
  const cardMap = useMemo(() => {
    ...
    const drawFitted = (img, rect, bg, fit = 'cover', customScale = 1) => {
      const rx = rect.x * W;
      const ry = rect.y * H;
      const rw = rect.w * W;
      const rh = rect.h * H;
      ...
      const scale = pick(rw / img.width, rh / img.height) * customScale;
    ...
    if (frontImage && frontTex.image) drawFitted(frontTex.image, FRONT_UV_RECT, null, imageFit);
    if (backImage && backTex.image) drawFitted(backTex.image, BACK_UV_RECT, 'white', 'contain', 0.5);
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current) {
      [j1, j2].forEach(ref => {
        ...
        if (!ref.current.lerped) ref.current.lerped = target.clone();
  ...
  return (
    <>
      <group position={[0, isMobile ? 6 : 4.5, 0]}>
        ...
        <RigidBody position={[1.8, 0, 0]} ref={card} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
          <CuboidCollider args={[2.4, 2.5, 0.03]} position={[0, -1.2, -0.05]} />
          <group
            scale={3} 
            position={[0, -1.2, -0.05]}
